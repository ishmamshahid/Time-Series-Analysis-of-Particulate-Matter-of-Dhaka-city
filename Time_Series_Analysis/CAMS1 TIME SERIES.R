...
Weekly_Means_PM10 <- aggregate(cams1_data["PM10"], format(cams1_data["date"],"%Y-%W"),
                          mean, na.rm = TRUE)
# derive the proper sequence of dates
Weekly_Means_PM10$date <- seq(min(cams1_data$date), max(cams1_data$date), length = nrow(Weekly_Means_PM10))
# plot the means
plot(Weekly_Means_PM10$date, Weekly_Means_PM10[, "PM10"],
     type = "l",
     lwd = 1.5,
     pch = 16,
     col = "darkorange2",
...
# calculate weekly means
Weekly_Means_PM2.5 <- aggregate(cams1_data["PM2.5"], format(cams1_data["date"],"%Y-%W"),
                               mean, na.rm = TRUE)
# derive the proper sequence of dates
Weekly_Means_PM2.5$date <- seq(min(cams1_data$date), max(cams1_data$date), length = nrow(Weekly_Means_PM2.5))
# plot the means
plot(Weekly_Means_PM2.5$date, Weekly_Means_PM2.5[, "PM2.5"],
     type = "l",
     lwd = 1.5,
     pch = 16,
     col = "darkred",
...
PM10.ts <- ts(Weekly_Means_PM10$PM10,freq=365.25/7, start=decimal_date(ymd("2013-01-01")))
PM2.5.ts <- ts(Weekly_Means_PM2.5$PM2.5,freq=365.25/7, start=decimal_date(ymd("2013-01-01")))

rm(PM2.5.ts.actual)


PM10.ts.train<-window(PM10.ts, start = 2013, end = 2018)
PM10.ts.test<-window(PM10.ts, start = 2018, end = 2019)
PM2.5.ts.train<-window(PM2.5.ts, start = 2013, end = 2018)
PM2.5.ts.test<-window(PM2.5.ts, start = 2018, end = 2019)
?window


plot(PM10.ts,ylim = c(0, 500))


plot(PM10.ts.train,ylim = c(0, 500))
