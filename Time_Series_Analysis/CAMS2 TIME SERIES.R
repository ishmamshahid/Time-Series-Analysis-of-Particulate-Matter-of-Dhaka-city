...
plot(cams2_data$date, cams2_data$PM10, type = "l", xlab = "year", ylab = "PM10 ")
rm(Weekly_Means)
...
Weekly_Means_PM10_2$date <- seq(min(cams2_data$date), max(cams2_data$date), length = nrow(Weekly_Means_PM10))
# plot the means
...
Weekly_Means_PM2.5_2 <- aggregate(cams2_data["PM2.5"], format(cams2_data["date"],"%Y-%W"),
                                mean, na.rm = TRUE)
# derive the proper sequence of dates
Weekly_Means_PM2.5_2$date <- seq(min(cams2_data$date), max(cams2_data$date), length = nrow(Weekly_Means_PM2.5))
# plot the means
...
PM10.ts.actual<-window(PM10.ts, start = 2018, end = 2019)
